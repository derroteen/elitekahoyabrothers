...
import { addLoanPayment, deleteLoan as deleteLoanFn } from "@/lib/loan.functions";
...
function LoansAdmin() {
  ...
  const doDeleteLoan = useServerFn(deleteLoanFn);
  ...
  return (
    <div>
                ...
                <tr key={l.id} className="border-b border-border last:border-0 hover:bg-muted/30">
                  ...
                  <td className="px-3 py-3 text-right whitespace-nowrap">
                    ...
                    {canEdit && (l.status === "approved" || l.status === "active" || l.status === "overdue" || l.status === "completed_with_fine") && (
                      <Button size="sm" variant="ghost" onClick={() => setRepayFor(l)}>Make Repayment</Button>
                    )}
                    {role === "super_admin" && (
                      <Button size="sm" variant="ghost" className="text-red-600 hover:text-red-700 hover:bg-red-50" onClick={() => setDeleteFor(l)}>DELETE LOAN</Button>
                    )}
                  </td>
                </tr>
              ))}
      ...
      {deleteFor && <DeleteLoanDialog loan={deleteFor} deleting={deleteLoan.isPending} onClose={() => setDeleteFor(null)} onDelete={() => deleteLoan.mutate(deleteFor.id, { onSuccess: () => setDeleteFor(null) })} />}
    </div>
  );
}

function DeleteLoanDialog({ loan, deleting, onClose, onDelete }: any) {
  return (
    <Dialog open onOpenChange={onClose}>
      <DialogContent>
        <DialogHeader><DialogTitle className="font-serif">Delete Loan</DialogTitle></DialogHeader>
        <div className="text-sm text-muted-foreground">
          Are you sure you want to permanently delete this loan?
        </div>
        <div className="text-xs font-mono text-muted-foreground">
          {loan.profile?.full_name ?? loan.member_id} · {fmtKES(loan.amount_borrowed)}
        </div>
        <DialogFooter>
          <Button variant="ghost" onClick={onClose} disabled={deleting}>Cancel</Button>
          <Button onClick={onDelete} disabled={deleting} className="bg-red-600 text-white hover:bg-red-700">{deleting ? "Deleting…" : "Delete Loan"}</Button>
        </DialogFooter>
      </DialogContent>
    </Dialog>
  );
...
function RepaymentDialog({ loan, onClose, onSaved }: any) {
  const doAddPayment = useServerFn(addLoanPayment);
  const [form, setForm] = useState({
    amount: "",
  ...
  const submit = async () => {
    ...
    setSubmitting(true);
    try {
      const r = await doAddPayment({ data: { loan_id: loan.id, amount: Number(form.amount), payment_date: form.payment_date, payment_method: form.payment_method, reference: form.reference || null, notes: form.notes || null } }) as any;
      toast.success(`Repayment recorded · ${r?.installments_covered ?? 0} installment(s) covered${Number(r?.fine_paid ?? 0) > 0 ? `, ${fmtKES(r.fine_paid)} fines paid` : ""}`);
      onClose(); onSaved();
    } catch (err: any) {
    ...
    <Dialog open onOpenChange={onClose}>
      <DialogContent>
        ...
        <div className="grid grid-cols-2 gap-3">
          ...
          <div className="col-span-2"><Label>Reference</Label><Input value={form.reference} onChange={(e) => setForm({ ...form, reference: e.target.value })} placeholder="Receipt, M-Pesa, cheque or bank reference" /></div>
